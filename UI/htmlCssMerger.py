    ...
    for line in css_like_lines:
        try:
            ...
            if match:
                ...
                if DEBUG:
                    print("\033[94m{}\033[00m".format("processing class name: "+suspected_name),)

                if org_name.find("auto layout")!=-1 or org_name.find("inside auto layout")!=-1 or org_name.find("identical to box")!=-1 or org_name.find("or ")!=-1:
                    if DEBUG:
                        print("\033[92m{}\033[00m".format("skiped Figma auto comments: "+suspected_name),)
                ...
                if current_class_name not in css_dict:
                    ...
                    print("\033[93m{}\033[00m".format("ERROR: duplicate CSS class name: "+current_class_name),)
                ...
                if style_match:
                    ...
                    if css_count == 1 and (property_name == "width" or property_name == "position"):
                        continue
                    if property_name=="background" and property_value.startswith("url"):
                        #check for image file in img folder
                        org_file = property_value.split("url(")[1].split(")")[0]
                        img_file = current_class_name+".png"#property_value.split("url(")[1].split(")")[0]
                        property_value =  "url(img/"+img_file+ property_value.split(org_file)[1]
                        if not os.path.exists(FOLDER_PATH+"/img/"+img_file):
                            print("\033[91m{}\033[00m".format("ERROR: background file not found: "+property_value+" for "+current_class_name),)
            ...
            print("\033[91m{}\033[00m".format("ERROR: processing css input at line "+line_num+": "+line),)
            print(e)
            exit(1)
...
def update_html_with_css(html_lines, css_dict):
    ...
    for line in html_lines:
        try:
            ...
            if class_match:
                class_name = class_match.group(1).strip()  # Normalize class name
                class_name = class_name[0].lower() + class_name[1:]  # Lowercase the first letter
                inline_style = class_match.group(2)

                if class_name not in found_html_names:
                    found_html_names.add(class_name)
                else:
                    print("\033[95m{}\033[00m".format("ERROR: duplicate HTML class name: "+class_name))

                # Try to find the class name in the CSS dictionary
                if class_name in css_dict:
                    if img_match and "background" in css_dict[class_name] and "url" in css_dict[class_name]["background"]:
                        print("WARNING: remove background:url(...) from img ele:"+class_name)
                        del css_dict[class_name]["background"]
                    new_style = css_dict[class_name]
                    new_style = '; '.join([f'{k}: {v}' for k, v in new_style.items()]) + ";"
                    # Replace the old style with the new updated style
                    updated_line = re.sub(r'style=".*?"', f'style="{new_style}"', line)
                    updated_line = '<div class="'+class_name+'" style='+updated_line.split("style=")[1]
                    updated_lines.append(updated_line)
                else:
                    updated_lines.append(line)  # No match, keep the original line
                    print("\033[91m{}\033[00m".format("ERROR: style not found for: "+class_name))
            else:
                updated_lines.append(line)  # No inline style, keep the original line
            ...
            print("\033[91m{}\033[00m".format("ERROR: processing html input at line "+line_num+": "+line))
            exit(1)
...
updated_html_lines[0] = "<body style=" + updated_html_lines[0].split("style=")[1]
...
for ele in html_elements:
    if ele not in css_dict:
        continue
    del css_dict[ele]
    updated_html_lines.append("var "+ele+" = document.getElementsByClassName('"+ele+"')[0];\n")
